...
ころころKitでそのまま編集したい場合は、スロットを4つずつ連続させて
1グループにしてください（0〜3, 4〜7, ...）。この形から外れる構成でも
ファームウェアとしては動かせますが、設定画面のグループ表示とは一致しません。
...
- 分割キーボードでは、ジェスチャー認識は **central 側でだけ**ビルド・動作します。
  トラックボールが peripheral 側にある構成では、ZMK の input-split で入力を
  central へ中継し、listener（とこのチェーン）を central 側に置いてください。
- ころころKitと同梱プリセットは、**1グループ = 上下左右4方向 = 4スロット**を
  標準形として扱います。設定画面もこの形を前提に、4スロットずつを1つの
  グループとして表示します。複合ジェスチャー（例: 上→右、下→左）も
  devicetree を自分で書けば使えますが、単方向ジェスチャーより確定待ちや
  誤認識が増えやすいため、標準用途ではおすすめしていません。
- 既定のスロット数は24です。おすすめ初期設定が使うのはグループ1〜4の
  16スロットで、グループ5〜6の8スロットは予備として空けています。
  追加のジェスチャーレイヤーが欲しくなったときに、設定画面から
  グループ5/6をレイヤーへ割り当てて使えます。
