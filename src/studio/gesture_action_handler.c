...
static int handle_get_groups(jpttm_gesture_action_Response *resp) {
    ...
    /* The one thing the UI cannot work out for itself: how many layers exist
     * is in the keymap it already reads, but which of them are spoken for is a
     * decision only this board can state. */
